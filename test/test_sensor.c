...
TEST(Sensor, initializeSensor_InitializesSensorPin)
{
  ...
  TEST_ASSERT_EQUAL_INT(GPIO_INTR_ANYEDGE, g->intr_type);
  TEST_ASSERT_EQUAL_INT64(1ULL << PIN, g->pin_bit_mask);
  TEST_ASSERT_EQUAL_INT(GPIO_MODE_INPUT, g->mode);
  TEST_ASSERT_EQUAL_INT(GPIO_PULLUP_ENABLE, g->pull_up_en);
  TEST_ASSERT_EQUAL_INT(GPIO_PULLDOWN_DISABLE, g->pull_down_en);
}
...
TEST(Sensor, initializeTrigger_startTimer)
{
  ...
  TEST_ASSERT_EQUAL_PTR((void *)TRIGGERPIN, tc->create_args->arg);
